    ...
    <motion.div variants={pageTransition} initial="initial" animate="animate" exit="exit" className="px-4 sm:px-6 py-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {loading ? (
          ...
          <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6" layout>
            <AnimatePresence>
              {filtered.map(item => (
                <motion.div
                  key={item.id}
                ...
                >
                  <motion.div className="lumina-card p-3 overflow-hidden group hover:translate-y-[-4px] transition-all duration-300 border border-stone-200/80 dark:border-stone-800" whileHover={{ scale: 1.01 }}>
      ...
      <AnimatePresence>
        {lightbox && (
          ...
          >
            ...
            >
              <img src={lightbox} alt="Gallery" className="w-full max-h-[85vh] object-contain rounded-3xl shadow-2xl" />
              <button
                onClick={() => setLightbox(null)}
